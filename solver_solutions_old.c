...
bool find_solutions(const shapes_list *shapes_list, const uint8_t *board);
...
bool find_solutions(const shapes_list *shapes_list, const uint8_t *board)
{
    if (shapes_list == NULL)
    {
        solutions *new_sol = (solutions*) malloc(sizeof(solutions));
        new_sol->shape = NULL;
        new_sol->next = sols;
        sols = new_sol;
    ...
    while (current_shape != NULL)
    {
        ...
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                ...
                {
                    bool sol_in_branch = find_solutions(shapes_list->next, new_board);
                    if(sol_in_branch)
                    {
                        solutions *new_sol = (solutions*) malloc(sizeof(solutions));
                        new_sol->shape = current_shape;
                        new_sol->x = x;
                        new_sol->y = y;
                        new_sol->next = sols;
                        sols = new_sol;
...
struct solutions_list *unwrap_solutions(){
